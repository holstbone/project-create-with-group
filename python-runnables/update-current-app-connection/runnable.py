...
class MyRunnable(Runnable):
    ...
    def run(self, progress_callback):
        ...
        PROPERTY_KEY="dku.security.allowedInApps"
        ...
        for connection_name in admin_client.list_connections():
            ...
            updated_connection_info = project_utils.update_connection_properties(connection_dict, group_names, project.get_summary()["projectKey"],PROPERTY_KEY)
            ...
            try:
                allowed_in_projects_property_orig = next(
                    (prop for prop in orig if prop['name'] == PROPERTY_KEY), None)
                
                allowed_in_projects_property_upd = next(
                    (prop for prop in upd if prop['name'] == PROPERTY_KEY), None)
                
                if allowed_in_projects_property_upd and not allowed_in_projects_property_orig:                
                    action="UPDATE"
                                
                elif not allowed_in_projects_property_upd and not allowed_in_projects_property_orig:                
                    action="NONE"
                    
                elif (allowed_in_projects_property_orig ):
                    orig=allowed_in_projects_property_orig["value"]

                    if (allowed_in_projects_property_upd):
                        if (len(orig)>0 and len(upd)>0):
                            upd=allowed_in_projects_property_upd["value"]
